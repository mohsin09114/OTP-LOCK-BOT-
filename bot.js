...
const start = async () => {
  ...
  const dropNumber = async (context) => {
    ...
    const botToken = 
  '6670283371:AAEeP-WnGjAoEHtHCEaQlNbtXamjbtwxtmU';
...
bot.onText(/\/start/, (msg) => {
  ...
  const videoPath = path.join(__dirname, 'files', 'Mohsin.mp4');
...
bot.onText(/\/temp (.+)/, (msg, match) => {
  ...
  const imagePath = path.join(__dirname, 'files', 'mohsin.png');
